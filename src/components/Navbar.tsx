...
const Navbar: FC<navbarprops> = () => {
  ...
  return (
    <>
      <nav className="w-full h-14 bg-white shadow  flex items-center text-center justify-between px-5 md:px-16 ">
        <div>
          <Link href="/">
        ...
        <nav className="hidden md:flex items-center justify-center list-none gap-5 ">
          ...
          <li className="hover:text-red-500">
            <button className="border bg-black text-white ml-5 border-gray-500 hover:bg-white hover:text-black px-4 py-1 uppercase rounded " onClick={onModalHandler}>
              Sign in
            </button>
          </li>
        </nav>
        <div className="flex md:hidden items-center justify-center gap-5 ">
          <span className="hover:text-red-500 text-center ">
            <button className="border bg-black text-white border-gray-500 hover:bg-white hover:text-black px-3 py-0.5  uppercase rounded " onClick={onModalHandler} >
              Sign in
            </button>
          </span>
          <span
              ...
              <div>
                <div className=" text-cente  ">
                  <div className="pt-8 pb-2 text-center flex items-center justify-around px-4 md:px-8   ">
                    <div className={`border w-full font-semibold md:text-base py-2 cursor-pointer ${loginTogle===true ? 'bg-blue-500 text-white ':'bg-gray-300'} `} onClick={()=>setLoginTogle(true)} >
                      Sign In
                    </div>
                    <div className={`border w-full font-semibold md:text-base py-2 cursor-pointer ${loginTogle===false ? 'bg-blue-500 text-white ':'bg-gray-300'} `} onClick={()=>setLoginTogle(false)} >
                      Sign Up
                    </div>
                  </div>
                  {
                    loginTogle ?
                    <form onSubmit={(e) => onSubmitHandler(e)} className=" px-2 md:px-10">
                   ...
                   <div className="flex flex-col text-left py-2 px-4 gap-1 mt-auto ">
                    ...
                    <div className="flex items-center justify-center flex-col py-5 gap-2 ">
                      <span className="text-sm font-bold text-green-600 cursor-pointer hover:underline " onClick={()=>setLoginTogle(false)}>Why Create an Account ?</span>
                      <p className="text-center text-[10px] ">By creating this account, you agree to our <span className="text-green-600">Privacy Policy</span> &<span className="text-green-600"> Cookie Policy.</span></p>
                    </div>
                  </form>:<form onSubmit={(e) => onSubmitHandler(e)} className="  px-2 md:px-10">
                  <div className="flex flex-col text-left py-2 px-4   gap-1  ">
                   ...
                   <div className="flex flex-col text-left py-2 px-4 gap-1 mt-auto ">
                    ...
                    <div className="flex items-center justify-center flex-col py-2 gap-2 ">
                      <span className="text-sm font-bold text-green-600 cursor-pointer hover:underline " onClick={()=>setLoginTogle(true)} >Already have an Account ?</span>
                      <p className="text-center text-[10px] ">By creating this account, you agree to our <span className="text-green-600">Privacy Policy</span> &<span className="text-green-600"> Cookie Policy.</span></p>
                    </div>
                  </form>
                  }
